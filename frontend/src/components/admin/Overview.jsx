...
const Overview = ({ 
  ...
  const solvedPercentage = totalTickets > 0 
    ...
    : 0;
  const pendingPercentage = totalTickets > 0 
    ? Math.round((pendingTickets / totalTickets) * 100) 
    : 0;
  const matchedPercentage = totalTickets > 0 
    ? Math.round((matchedTickets / totalTickets) * 100) 
    : 0;
        ...
        <div className="bg-green-50 p-4 rounded-lg shadow-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-3xl font-bold text-green-800">
              {solvedTickets} <span className="text-sm text-green-600">({solvedPercentage}%)</span>
            </span>
          </div>
          <h3 className="text-md font-semibold text-green-600">Solved Tickets</h3>
        </div>
        ...
        <div className="bg-yellow-50 p-4 rounded-lg shadow-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-3xl font-bold text-yellow-800">
              {pendingTickets} <span className="text-sm text-yellow-600">({pendingPercentage}%)</span>
            </span>
          </div>
          <h3 className="text-md font-semibold text-yellow-600">Pending Tickets</h3>
        </div>
      {/* ticket match */}
      <div className="bg-blue-50 p-4 rounded-lg shadow-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-3xl font-bold text-blue-800">
              {matchedTickets} <span className="text-sm text-blue-600">({matchedPercentage}%)</span>
            </span>
          </div>
          <h3 className="text-md font-semibold text-blue-600">Matched Tickets</h3>
        </div>
      ...
      <div className="bg-red-50 p-4 rounded-lg shadow-md">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-3xl font-bold text-red-800">
              {unmatchedTickets} <span className="text-sm text-red-600">({unMatchedPercentage}%)</span>
            </span>
          </div>
          <h3 className="text-md font-semibold text-red-600">Unmatched Tickets</h3>
        </div>
